...
- Changed: **`task-convention.md`**: one work tree with six stored kinds, one home and parent, deadline rows and wall-map revisions, derived action dates, review state, refusal checklist, typed close dispositions, atomic execution claims, and the expanded verb/view contract. Existing annotations, ranking, append-only history, ingest, backup, and degraded markdown remain supported.
- Added: **`human-agent-collaboration.md` §Two resident agents**: coordinator/engineer roles, durable dispatch classes and briefs, worktree isolation, integration, cross-family review and two-round cross-examination, shared institutions, and nonblocking ledger messages. Changed: **§Standardized choice surfaces / Action staging**: typed intake, ordered executor-marked plans, waivable review with gates preserved, execution, parking, and recurring-branch formalization.
- Added: **`portable-skills.md` §One source, scoped delivery**: declared consumers, native links, no same-name shadowing, project parameters, exported snapshots, and computed description caps. Changed: **project-founding size test** to use the task model for both ending and standing project homes.
- Added: **`session-discipline.md` §The session record**: item intake and transitions, evidence labels, interjections and pivots, standing orders, code-rendered closing blocks, and stop-hook checks. Changed: **Reply format / Wrap protocol** to use the record and park user waits on task lines. Added: **§Session notifier** with the Stop/Notification policy: completion at 120 seconds or longer, mid-turn dialog idle at 60 seconds with no Stop for the latest prompt, immediate permission alerts, and silent post-turn idle.
- Added: **`agent-reliability.md` §One permission policy**: one host-mapped policy, destructive-operation denies, protected zones, final-press asks, live-session guards, enabled sandbox/approval controls, and a read-only config audit. Practice-derived policy is distinguished from the existing cited literature.
- Changed: **`engineering-standards.md` §Testing**: controls before tests, smoke test and standard pytest block at package birth, fast keyless defaults, seam tests and fail-closed tag gates, narrow CI, regression checks, and independent substantive-build review.
- Added: **`autoflow.md` §Isolate, then promote**: one workspace/branch/ledger row per unit, serialized promotion with post-rebase whole-state checks, escalation for changes to the checks themselves, per-action authority, and promote-or-discard within the cycle. With ample budget, WIP and verified-merge capacity govern dispatch; spare compute funds diverse redundant verification.
- Changed: **`skeleton/store/schemas/schemas.yaml`**: replaces `work_item` with `tasks`, `deadlines`, `dispositions`, and `claims`, keeping judgment fields separate and declaring task vocabularies and refusal semantics. **`skeleton/hub/_forms/campaign.md` and `skeleton/hub/_forms/playbook.md`** gain parent references and generated-field guidance; the playbook explicitly has no finishing end while its scope holds. Corpus/skeleton indexes, the build log, and the release plan are aligned.
- Installed-estate migration: regenerate the affected instruction and skill layers from the existing adaptation ledger, including scoped skill links and host-specific hook/policy mappings where supported. Merge schema and form-template changes without overwriting living state. Rebuild the installed task tool against the new schemas at its next natural touch, preserving ids, ranks, pins, history, and the single writer; run the task convention's cutover checks. Markdown lists need no rewrite because the line conventions are additive; assemble deadline rows for existing dated items at the next wall-map pass. Runtime code and platform commands are built or adapted within the sanctioned installation, not shipped in this seed.
- Affects: derived layers of task-convention, human-agent-collaboration, portable-skills, session-discipline, agent-reliability, engineering-standards, and autoflow; task schemas and lazy-founded campaign/playbook templates. MAJOR: task-model and schema restructuring; the installer and empty TODO templates retain their interfaces.
