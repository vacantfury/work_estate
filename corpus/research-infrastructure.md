...
- **Work stack** — for anything employer-confidential: the employer's sanctioned LLM endpoints, the employer's secret store, employer compute. Discovered before built (design principle 10): the team almost certainly already has incumbents — model gateways, eval harnesses, a secrets solution, experiment infrastructure — found at intake (§17–§18). The work stack's seam and compute client are chartered infrastructure projects — `llm_utils.md` and `devices.md` — each resolving incumbent-vs-build at intake inside its own charter (incumbents sit behind the seams, never replace them).
