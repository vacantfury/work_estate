...
- Sweep on session opening or on the engine's schedule (`work-engine.md` runtime adaptation decides which; degraded install: a session-opening checklist); the bus carries its own channel adapters — mail, chat, calendar — resolved at unfold (A8). Every new message lands in one of three lanes:
