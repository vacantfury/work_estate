...
- **Jot sections are ordinary files:** the agent or the user appends people met, ideas, and notables as they come up, at whatever grain is convenient. Full sentences preferred over fragments.

## The grab

`./grab` (run it on the user's ask — "refresh sidework" — or at the end of a working session; never on a schedule):
...
2. Print a short report of what arrived (new and updated files per section) — the report doubles as the user's what's-new view over their side work.
3. Where the user wants an aggregate page (e.g. all experiment results across repos on one page), the agent renders it into the relevant section on ask.

## Scope

- Sources are **side repos only** — the repos of the user's own projects present on this machine — and only their manifest-declared parts ("part of some repos" is the point: declare `results/` and `notes/`, not the whole tree).
...
The agent creates the folder, `grab.manifest`, and the `grab` script (a small wrapper implementing the two steps above) when the user first asks for sidework; the manifest starts from whichever side repos are present and the user's word on which parts matter.
