...
- **Exported snapshots are the sole copy exception.** An explicitly exported skill carries a canonical-source pointer and source revision in its frontmatter. It is a versioned snapshot, not a second live source. The seed's portable entries are such exported conventions; their installed adaptations acquire one canonical home within the installation. The pointer identifies the source appropriate to that installation, never a private source or hidden dependency.
- **Check native discovery.** Verify the sanctioned tooling discovers each intended link exactly once and exposes no out-of-scope skills. Where it needs a real directory of per-skill links, use that shape; do not assume a linked directory root or nested skills will resolve the same way across hosts.
- **Compute a description cap when tooling truncates.** Derive the per-skill ceiling from the largest visible consumer set and the tightest supported listing budget, including name/path overhead. Record the computed cap in config and flag over-cap descriptions. Keep the description to when-to-use; move detailed triggers into the body. Unknown host limits require a discovery check, never a guessed fixed cap.

At a copied skill's next touch, move reusable changes into its canonical source, move project facts into project config, then replace the copy with a link. With no skill-capable tooling, the same source serves as instructions or a printed checklist; no divergent procedure is maintained.
...
Trigger: a container is about to be created — a per-project home under main-work at an ACCEPT or an accepted initiative (`project-pipeline.md` stages 3/5), a new function project when a standing kind of work settles with no home, or an infrastructure charter's founding gate firing at its named first consumer. Also fires on discovery of homeless work: real work accumulating in sessions, scratch, or another project's files with no registered home.

0. **Size test first.** Use `task-convention.md` §The work tree: one action is a task; a coordinated effort is a campaign or playbook; work with its own project life earns a project home, either ending or standing. Duration never decides the form. Container bodies follow `estate-structure.md`.
1. **Classify the kind** (`estate-structure.md` kinds roster): per-project home under main-work (product work) · function project (a standing register/record of the job) · infrastructure (a seam ≥2 projects ride). Infrastructure founds ONLY at its charter's named consumer gate, per its own module (`store.md` · `finder.md` · `ties.md` · `llm_utils.md` · `devices.md` · `autoflow.md` · `auto_research.md`); a capability two projects both need but no charter covers is infrastructure debt — flag it at the hub, never hand-roll it inside one project.
...
3. **Copy the scaffold — structure is never re-derived by hand.** Per-project home: copy `main-work/_template/` whole (task surfaces + `record.md`, `project-home.md`). Function/infrastructure project: copy its skeleton directory (`skeleton/<name>/` in the seed; the estate's founded siblings are the live copies). An existing file is NEVER overwritten — founding over existing material is absorption (step 8).
4. **Fill the kind-specific state.** Per-project home: the record's Design section + the accept's premortem (`risk-register.md`) declared before work starts; the team's design-doc convention wins where one exists (`project-home.md`). Function project: its registers per its owning corpus module, in the data shape (`estate-structure.md`): typed entries · append-only dated assessments · machine estimates separate from the user's judgments · knobs in a config block. Infrastructure: per its charter module. Deliberately NOT pre-seeded: `lessons.md` and `handbook.md` found lazily at their first entry from the hub's `_forms/` — empty shells are clutter.
...
Since v3 the roster includes the infrastructure trio's connectors (store engine, finder sources, ties channels) and the seed's registry row — flag a stale update channel (newest version seen far ahead of installed, or last-checked ancient).
...
2. Check the calendar and inbox for anything that reorders today (work-sanctioned surfaces only) — the inbox check IS the triage sweep of `ties.md` (three lanes, drafts staged inert), never a parallel hand-rolled pass; the calendar read also feeds that module's meeting-prep assist, and its meeting list opens with the start-capture-before-entering front line (`ties.md` §Meetings capture bullet).
...
2. Desired outcome first: what result makes the meeting a success; then the user's asks, each carrying its context and a recommendation (the recipient-attention rule, `ties.md`).
...
4. **Strategies** — the intro line (lead with what that room values), per-target ask scripts, the on-site finding plan in numbered steps, and the after-event close: outcomes recorded per the meetings convention, follow-ups staged as drafts (`ties.md`).
